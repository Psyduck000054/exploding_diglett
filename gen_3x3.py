...
def gen_3x3(cell_list, cell_value, cell_terrain, rate, rng_system, block_rad=2, min_neighbours=0, transparency=False):
    for i in range(1, SIZE_X - 1):
        for j in range(1, SIZE_Y - 1):
            ...
            rng_val = rng_system.random()
            if ((cell.terrain in cell_terrain) and rng_val < rate) and alike_neighbour(cell_list, i, j, cell_terrain) >= min_neighbours:
                ...
                for x in range(i - 1, i + 2):
                    for y in range(j - 1, j + 2):
                        if cell_value == 10 and x == i and y == j:
                            cell_list[x][y].update_value(cell_value, rng_system=rng_system, force_terrain=majority_bg)
                        else:
